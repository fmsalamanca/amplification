...
path = 'C:\\Users\\Elastomeros Fernando\\Nextcloud\\Papers\\Entanglements\\Masticados-instron\\'+str(dirID)+'.is_tens_RawData\\'
...
for i in range(reps):
    ...
    Ncc  = rho*R*T*0.5/(gcdefff[i]*Ms) #Ncc is an array, so we select the component zero which is a float
    Nc.append(Ncc[0])
...
if saving:
    dirdest  = path+'Analysis-phantom'+str(dirID)
